...
const ResearchLab = () => {
  ...
  const copy = getResearchContent(locale).laboratories;

  return (
    <section className="relative scroll-mt-24 overflow-hidden bg-[#f7faff] px-4 py-16 sm:px-6 md:py-24 lg:px-8">
      <LabsAmbient />

      <div className="relative z-20 mx-auto max-w-7xl">
        <div className="mb-12 grid gap-8 lg:grid-cols-[minmax(0,1fr)_420px] lg:items-end">
          <div>
            ...
            <p className="mt-7 max-w-2xl text-justify text-base font-light leading-8 text-slate-600 sm:text-lg">
              {copy.description}
            </p>
          </div>
          ...
          <div className="grid grid-cols-3 overflow-hidden rounded-lg border border-blue-100 bg-white/88 shadow-[0_18px_55px_rgba(15,23,42,0.07)] backdrop-blur-xl">
            {[
              ...
              { value: "AI", label: copy.stats[1], icon: BrainCircuit },
              { value: "PhD", label: copy.stats[2], icon: UsersRound },
            ].map((stat, index) => (
              <div
                key={stat.label}
                className={[
        ...
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
          {laboratories.map((lab, index) => (
            <LaboratoryCard key={lab.id} lab={lab} index={index} copy={copy} />
          ))}
        </div>

        <div className="mt-10 flex flex-col items-start justify-between gap-5 rounded-lg border border-blue-100 bg-white/88 p-5 shadow-[0_18px_55px_rgba(15,23,42,0.07)] backdrop-blur-xl md:flex-row md:items-center">
...
function LaboratoryCard({ lab, index, copy }: { lab: Laboratory; index: number; copy: ReturnType<typeof getResearchContent>["laboratories"] }) {
  ...
  return (
    <article className="group relative flex min-h-[430px] flex-col overflow-hidden rounded-lg border border-blue-100 bg-white/88 shadow-[0_18px_55px_rgba(15,23,42,0.07)] backdrop-blur-xl transition duration-300 hover:-translate-y-1 hover:border-blue-200 hover:bg-white hover:shadow-[0_26px_80px_rgba(37,99,235,0.16)]">
      <Link
        href={lab.href}
        className="absolute inset-0 z-20"
        aria-label={localizedLab.name}
      />
      ...
      <div className="flex grow flex-col p-6">
        <h3 className="line-clamp-2 text-xl font-black leading-snug text-slate-950 transition group-hover:text-blue-700">
          {localizedLab.name}
        </h3>
